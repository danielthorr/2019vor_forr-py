...
def server(client_list):
    ...
    while True:
        ...
        # When we receive an incoming connection we set the target of the process to the handle_thread() function
        # we also send with it the empty client list, the client's socket object (conn) and the address bound to the socket
        p = multiprocessing.Process(target=handle_client, args=(client_list, conn, address))
